    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <form action="{{ route('exams.update', $exam) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" value="{{ $exam->name }}" required
                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>

            <div class="mb-4">
                <label for="date" class="block text-gray-700 text-sm font-bold mb-2">Date</label>
                <input type="date" name="date" id="date" value="{{ $exam->date->format('Y-m-d') }}" required
                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="professor_id" id="professor_id" required
                    ...
                    @foreach($professors as $professor)
                        <option value="{{ $professor->id }}" {{ $exam->professor_id == $professor->id ? 'selected' : '' }}>
                            {{ $professor->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="grade" id="grade" min="1" max="10" value="{{ $exam->grade }}"
                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
